...
function addCurrency(salaryStr, fullText = "") {
  ...
  if (/(دينار|ألف|الف|IQD|\$|دولار|USD|نسبة|نسب)/i.test(s)) return s;
...
export function smartSalary(text = "") {
  ...
  // التقاط أرقام قصيرة بعد كلمة الراتب مع السياق (600 وقابل للزيادة)
  for (const line of lines) {
    const m = line.match(/(?:الراتب|راتب|الأجر|الاجر|salary)\s*[:：\-–—]?\s*(?:(?:ابتداء|يبدأ|من)\s*(?:من)?\s*\(?\s*)?(\d{3,4}(?:\s*[-–—]\s*\d{3,4})?)(?:\)?\s*(وقابل للزيادة|قابل للزيادة|قابل للتفاوض|وقابل للتفاوض))?/i);
    if (m?.[1]) {
