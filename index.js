...
// Topic IDs for Forum/Topics groups (optional — set if group has Topics enabled)
const QUDRAT_TOPIC_ID = process.env.QUDRAT_TOPIC_ID ? Number(process.env.QUDRAT_TOPIC_ID) : null;
const REVIEW_TOPIC_ID = process.env.REVIEW_TOPIC_ID ? Number(process.env.REVIEW_TOPIC_ID) : null;
...
// Helper: send message with auto topic ID
async function tgSend(chatId, text) {
  const payload = { chat_id: chatId, text };

  // Add topic ID if the group has Topics/Forum enabled
  if (chatId === QUDRAT_CHAT_ID && QUDRAT_TOPIC_ID) {
    payload.message_thread_id = QUDRAT_TOPIC_ID;
...
function extractTelegramHandles(text = "") {
  const matches = normalizeText(text).match(/@[A-Za-z][A-Za-z0-9_]{3,}/g) || [];
  // Filter out common email-like patterns
...
function isLikelyPhone(s = "") {
  ...
  // Iraqi phones: +964..., 077..., 078..., 079... — at least 9 digits
  if (digits.length >= 9 && /^(\+?964|077|078|079|076|075|07[0-9])/.test(x)) return true;
  // Generic: 9+ digits likely phone
  if (digits.length >= 9) return true;
  return false;
}

function isLikelySalaryNumber(s = "") {
  // Extract all digit groups from the string
  const digits = normalizeInline(s).replace(/[^\d]/g, "");
  // Salary: max 7 digits (up to 9,999,999). Phone: 9+ digits
  return digits.length > 0 && digits.length <= 7;
}

function containsPhoneNumber(s = "") {
  const x = normalizeInline(s);
  // Check if string contains an Iraqi phone pattern anywhere
...
function isLikelySalaryValue(s = "") {
  ...
  // If the whole string is just a phone number, reject
  if (isLikelyPhone(x)) return false;
  // Check for salary-like patterns
  ...
  // If it contains an embedded phone, still might be salary with contact mixed in — we'll clean it later
  return true;
}

function cleanSalaryValue(s = "") {
  let x = normalizeInline(s);
  // Strip Iraqi phone numbers from salary text
  x = x.replace(/(?:\+?964|07[0-9])\s*\d[\d\s\-]{6,}\d/g, "").trim();
  // Strip generic long numbers (9+ digits) that look like phones
  x = x.replace(/\b\d{9,}\b/g, "").trim();
  // Strip contact info trailing text
...
function inferApplicationMethod(text = "", contact = "") {
  ...
  if (extractTelegramHandles(full).length > 0) return "تيليجرام";
...
function fallbackCategory(text = "", title = "") {
  ...
  if (/(admin|إداري|اداري|استقبال|رسبشن|سكرتير|سكرتارية|office|مدخل بيانات|مدخلة بيانات|data entry|إدخال بيانات|ادخال بيانات)/i.test(s)) return "Admin";
  if (/(hotel|مطعم|مقهى|barista|chef|hospitality|ضيافة|طباخ|سياحة|سياحي|سياحية|سفر|travel|tourism|فندق|فندقي|فندقية|OTA|كيمنك|gaming|بلي بنت|بلي ستيشن|playstation|ألعاب|العاب|صالة ألعاب|صالة العاب|نادي|ترفيه|entertainment)/i.test(s)) return "Hospitality";
  ...
  if (/(marketing|تسويق|مروج|مروجة|designer|تصميم|مصمم|جرافيك|مونتاج|مونتير|مصور|مصورة|كاتب محتوى|كاتبة محتوى|صناعة محتوى|صانع محتوى|content|سوشيال ميديا|social media)/i.test(s)) return "Marketing";
...
// Non-job filter
// =========================
function isNotJobAd(text = "") {
  const x = normalizeText(text).toLowerCase();
  // Commercial ads, admin announcements, etc.
  ...
  // Very short texts unlikely to be job ads
...
// Processing Queue
// =========================
const QUEUE_DELAY_MS = Number(process.env.QUEUE_DELAY_MS || 3000);
...
// =========================
// Ad Processing (extracted from webhook)
// =========================
async function processAd({ rawAdId, rawText, cleanText, hash }) {
  ...
  if (decision.bucket === "QUDRAT") {
    ...
    const tgRes = await tgSend(targetChatId, finalText);
...
// =========================
// Webhook (lightweight — save + enqueue)
// =========================
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Enqueue for processing instead of processing immediately
...
// =========================
// Health + Queue Status + Diagnostics
// =========================
app.get("/", (_req, res) => {
...
// Test endpoint: sends a test message to QUDRAT to verify permissions
app.get("/test-publish", async (_req, res) => {
  const testText = `🧪 رسالة اختبار - ${new Date().toISOString()}\nهذه رسالة تجريبية للتأكد من صلاحيات البوت.`;

  console.log("TEST: sending to QUDRAT_CHAT_ID:", QUDRAT_CHAT_ID);
  const tgRes = await tgSend(QUDRAT_CHAT_ID, testText);

  console.log("TEST RESULT:", JSON.stringify(tgRes, null, 2));

  res.json({
    target_chat_id: QUDRAT_CHAT_ID,
    topic_id: QUDRAT_TOPIC_ID,
    telegram_response: tgRes,
    success: tgRes?.ok || false
  });
});

// Test: sends a test message to REVIEW
app.get("/test-review", async (_req, res) => {
  const testText = `🧪 رسالة اختبار مراجعة - ${new Date().toISOString()}\nهذه رسالة تجريبية.`;

  console.log("TEST: sending to REVIEW_CHAT_ID:", REVIEW_CHAT_ID);
