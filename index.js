...
function smartTitle(text = "") {
  ...
  // 1) صيغ واضحة "المسمى الوظيفي/الوظيفة"
  let raw =
    (t.match(/(?:المسمى\s*الوظيفي|المسمى|الوظيفة|Job\s*Title)\s*[:：\-–—]\s*([^\n]{3,120})/i)?.[1]?.trim()) ||

    // 2) "مطلوب/مطلوبة/نبحث عن" + قد يجي بعدها الجنس ثم العنوان
    (t.match(/(?:مطلوب|مطلوبة|نبحث عن)\s+([^\n]{3,160})/i)?.[1]?.trim()) ||

    // 3) fallback آخر: أول سطر مفيد
    firstLine(text);

  raw = raw || "";

  // إزالة الإيموجي والرموز الزائدة
  raw = raw.replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/gu, "").trim();

  // شيل الكلمات اللي تصف الجنس/التوظيف العام وتخلي العنوان فقط
  raw = raw
    .replace(/\b(ذكور|إناث|للجنسين|جنسين|موظف(?:ين)?|موظفة|موظفات|موظفين|عامل(?:ين)?|عاملة|كادر|كوادر)\b/gi, "")
    .replace(/\b(حصراً|فقط|عدد|مطلوب)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();

  // اقطع عند كلمات تبدأ تفاصيل وليست عنوان
  raw = raw.replace(
    /\s+(?:شركة|اسم الشركة|الراتب|الدوام|اوقات|الموقع|العنوان|للتقديم|التقديم|التواصل|واتساب|مزايا|شروط|المهام|تفاصيل)\b.*$/i,
    ""
  );

  // اقطع عند الفواصل الطويلة
  raw = raw.replace(/[|،\-–—].*$/i, "").trim();

  // إذا بقي فارغ رجّع غير مذكور
  return raw || "غير مذكور";
}
// ===== Webhook =====
