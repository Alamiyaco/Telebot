...
async function handleEditCallback(cbq) {
  ...
  const originalMsgId = cbq.message?.message_id;
  const topicId = cbq.message?.message_thread_id || 0;
  ...
  if (prompt?.ok) {
    ...
    setTimeout(() => pendingEdits.delete(prompt.result.message_id), 5 * 60 * 1000);
  }
}
...
async function handleEditReply(msg) {
  ...
  const { pubId, field, originalMsgId, chatId, topicId } = editInfo;
  const fieldInfo = EDIT_FIELDS[field];
  if (!fieldInfo) return;
  ...
  try {
    await tg("deleteMessage", { chat_id: chatId, message_id: promptMsgId });
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // === معالجة ردود التعديل في كروب قدرات ===
    ...
    if (decision.bucket === "QUDRAT") {
      ...
      const tgRes = await tg("sendMessage", {
        ...
        text: finalText,
        reply_markup: buildEditKeyboard(0), // سنحدث الـ ID بعد الحفظ
        ...(topicId ? { message_thread_id: topicId } : {})
      });
      ...
      // تحديث أزرار التعديل بالـ ID الصحيح
