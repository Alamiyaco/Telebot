...
async function tgSend(chatId, text, category) {
  ...
  if (chatId === QUDRAT_CHAT_ID) {
    ...
    // null = General (send without thread ID), number = specific topic
    if (topicId) {
...
async function processAd({ rawAdId, rawText, cleanText, hash }) {
  ...
  if (decision.bucket === "QUDRAT") {
    // Build list of ads to publish
    let adsToPublish = [];

    if (finalResult.is_multi_role && finalResult.roles && finalResult.roles.length > 0) {
      // Split: create separate ad for each role with shared details
    ...
    for (const ad of adsToPublish) {
      ...
      const tgRes = await tgSend(targetChatId, finalText, ad.category);
      ...
      `).run(
        ...
        ad.application_method || "غير مذكور",
