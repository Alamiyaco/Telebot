...
function fallbackCategory(text = "", title = "") {
  ...
  if (/(marketing|تسويق|مروج|مروجة|designer|تصميم|مصمم|جرافيك|مونتاج|سوشيال ميديا|مصور|تصوير|فوتوغراف|فيديو|كاتب محتوى|كتابة محتوى|content|صانع محتوى|ريلز|reels|إعلانات|ميديا|media|creative|إبداعي)/i.test(s)) return "Marketing";
...
8) salary لا تستنتجه من رقم هاتف أو رقم عشوائي.
9) category "Marketing" يشمل: التسويق، التصميم الجرافيكي، المونتاج، التصوير، كتابة المحتوى، السوشيال ميديا، صناعة المحتوى، الريلز، والإعلانات. لا يوجد تصنيف "Design" منفصل.
9) إذا الإعلان يحتوي أكثر من وظيفة واضحة ضع is_multi_role = true.
10) confidence رقم من 0 إلى 1.
11) summary مختصر جداً، سطر واحد فقط، دون اختراع أي معلومة.
12) إذا كانت التلميحات الأولية صحيحة فاستخدمها، وإذا كانت خاطئة تجاهلها. التلميحات ليست حقائق ملزمة.
13) أعد JSON فقط.
                `.trim()
              }
            ]
    ...
    if (decision.bucket === "QUDRAT") {
      ...
      db.prepare(`
        INSERT INTO ads_published (
          ...
          confidence, raw_text, clean_text,
